...
% In fact, it has been reported that attending to the duration of a stimulus leads to perceiving it as longer~\cite{Coull2004Sci}.
...
\footnotetext{
    ...
    \Citeauthor{Shadmehr2019TINS} defined \emph{economic utility} as ``a measure of how much one values a particular good", i.e., the subjective value of outcome~\cite{Shadmehr2019TINS}.
...
Although the capture rate ostensibly depends on three variables, in practice those variables are not mutually independent.
...
In principle, the purpose of any goal-directed movement is assumed to be transitioning to a more rewarding state, then, due to temporal discounting of rewards, the duration of movement per~se incurs a cost by postponing the reward acquisition.
This is called the \gls{cot} hypothesis.
\par
The concept of the \gls{cot} has been useful for understanding why we don't move slower~\cite{Berret2016JNeurosci}.
Moving fast is appealing due to lower \gls{cot}, however, faster movements are bounded by both energetic demand\cite{Long2013RoSocInterface} and speed/accuracy trade-off~\cite{Harris2006BioCyber}.
On the other hand, moving slowly imposes \gls{cot} and that might be why humans are reluctant to move their arms slowly~\cite{Berret2018SciReports}.
Thus, optimal control framework has been utilized to infer the shape of the \gls{cot} as a function of movement time.
...
Similarly, \gls{cot} seems to account for inter-individual differences in \emph{vigor}\footnotemark~\cite{Berret2018SciReports}.
\footnotetext{
    A key parameter of movement is its vigor, that is, its speed, amplitude, or frequency. Movement vigor is generally thought of as something that varies with motivation. [Dudman2016]
    
    We use ``movement vigor" to refer to a general property of movement kinematics reflected in various, often correlated measurements (eg, speed and amplitude). This is by analogy to descriptions of “response vigor” that refer to a general property of task engagement indexed by a specific measurement (eg, rate of lever pressing) [Yttri2018]
    }
CONTINUE ON INTER-INDI DIFFERENCES + SIMILAR SHADMEHR RESULTS
