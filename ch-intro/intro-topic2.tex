...
Optimal foraging proposes a kind of `currency' with evolutionary advantages to behave in a way that maximizes it~\cite{Shadmehr2019TINS,Carland2019NeuroSci}.
This currency is called \emph{capture rate} or \emph{reward rate} and in principle, is defined as the sum of the acquired rewards, minus cost of action, divided by total elapsed time~\cite{Shadmehr2019TINS}.
Although the capture rate seemingly depends on 3 variables, in practice it is more complicated, because those variables are not mutually independent.
...
\par
In the context of time perception, since animals seem to default to motor activity to estimate time, optimizing the capture rate could be deemed a natural and intuitive policy.
Consequently, to optimize the capture rate, the time interval should be substituted with a motor program that has two features:
...
\footnotetext{
    ...
    Reward evaluation and its impact of choice is a topic in decision-making and out of scope of this work.
    }
Furthermore, cost of action mostly translates to metabolic cost.

RELATION WITH VIGOR AND LOCMOTION ...