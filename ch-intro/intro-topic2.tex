...
Optimal foraging proposes a kind of `currency' with evolutionary advantages to behave in a way that it remain maximized~\cite{Shadmehr2019TINS,Carland2019NeuroSci}.
This currency is \emph{capture rate} or \emph{reward rate} and in principle, is defined as the sum of the acquired rewards, minus cost of action, divided by total elapsed time~\cite{Shadmehr2019TINS}.
Although the capture rate ostensibly depends on 3 variables, in practice it is more complicated, because those variables are not mutually independent.
For instance, cost of action mostly translates to metabolic cost, which is directly related to the speed of movement.
...
Faster movements finish earlier, i.e., shorter elapsed time, and therefore higher capture rates!
\par
In the context of time perception, since animals seem to default to a motor activity to estimate time, optimizing the capture rate could be deemed a natural and intuitive policy.
Consequently, to optimize the capture rate, the time-keeping during the interval should be substituted with a motor program that has two features:
    1)~Its execution lasts the same duration as the time that needs to be estimated, hence, maximizing the reward\footnotemark and minimizing the denominator of the capture rate;
    2)~Its execution is the least costly.
\footnotetext{
    ...
    \Citeauthor{Shadmehr2019TINS} defined \emph{economic utility} as ``a measure of how much one values a particular good"~\cite{Shadmehr2019TINS}.
    Reward evaluation and its impact on choice is a topic in decision-making and out of the scope of this work.
    }
By definition, the least costly action is the least effortful one.
% More effortful movements are known to be slower and with higher reaction times.
Faster movements increase the metabolic cost, which is an objective measure of effort.
Individuals have preferred speed...:(