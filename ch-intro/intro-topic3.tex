...
For instance, \Citeauthor{Barnes2005Nature} showed that \glspl{msn} in the \gls{dls} fired sharply at the start and end of an overlearned run trajectory in a T-maze~\cite{Barnes2005Nature}.
This `chunking' was then proposed to mediate the process of acquiring motor habits.
In another study mice were trained to press a lever 8 times to receive a reward (fixes-ration schedule)~\cite{Jin2010N}.
...
This pattern grew stronger with learning.
Thus they also concluded that the striatum is important in learning action sequences and their initiation~\cite{Jin2010N}.
However, further experiments indicated that the activity profile of \glspl{msn} is more complex.
Some neurons displayed sustained activity during the sequence in both direct and indirect pathways~\cite{Jin2014NN}.
Moreover, calcium transient recorded from \gls{d1} and \gls{d2} striatal neurons demonstrated concurrent activity in both cell types during movement~\cite{Cui2013Nature}.
Taken together, these results support the action selection model of the striatum, according to which the direct pathway promotes the selected action while the indirect pathway inhibits the competing alternatives.
Nevertheless, there is also evidence indicating that the striatal activity may form a continuous space and functional clusters may reflect instantaneous movement information, rather than discrete start and stop signals~\cite{Sales-Carbonell2018, Markowitz2018Cell}.
