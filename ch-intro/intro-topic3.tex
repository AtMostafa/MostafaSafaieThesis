...
With the strictest clustering criteria, \citeauthor{Hunnicutt2016} identified two areas, which map roughly to the \gls{dls} and \gls{dms}~\cite{Hunnicutt2016}.\footnotemark\
\footnotetext{
    \Gls{dls} and \gls{dms} correspond to the putamen and caudate nuclei, respectively.
    In primates, the internal capsule divides the striatum in two halves, thus the use of caudate-putamen nucleus instead of the striatum is more common (see \autoref{fig:intro:InputMap}).
    }
...
In the course of motor habit formation, as goal-directed behaviors are becoming habitual, it is suggested that the \gls{dls} takes over the \gls{dms}'s role.
...
Importantly, the \gls{dls} expands medially much larger than what has been traditionally regarded as the \gls{dls}, suggesting that areas that has been considered as the \gls{dms} in the literature, partially include the \gls{dls} too.
...
\Citeauthor{Mink1996} as well as other researchers proposed the action selection model, according to which the two pathways may work in concert to support the desired actions (direct pathway) and suppress the unwanted competing ones (indirect pathway)~\cite{Mink1996}.
...
In the condition wherein tokens jumped the fastest post-decision, unsurprisingly, the ratio of hasty decisions increased and animals made faster movements too~\cite{Thura2014JNeurosci}.
