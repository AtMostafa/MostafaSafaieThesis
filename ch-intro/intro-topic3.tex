...
\footnotetext{
    ...
    In a complex dynamical system such as the brain, by transiently changing the activity of the area of interest, off-target downstream structures are driven into a unnatural state that could have behavioral implications of its own~\cite{Otchy2015Nature}.
    }
...
Importantly, in many of those tasks, including the two mentioned above, reduced vigor could also be due to overestimation of the cost as well.
\par
...
They extensively trained monkeys to guess which of the two potential targets would receive the majority of tokens that jumped from a central point to one of the possible targets every 200~ms.
Crucially, after the subject reached the target, the rest of the tokens jump more quickly, thus presenting the monkey with a trade-off to either make confident decisions or guess earlier and receive potential rewards sooner.
How much faster the tokens jump after the decision (every 150~ms, or every 50~ms), compared to their normal pace (every 200~ms) determines different speed-accuracy trade-off policies, i.e., the faster they jump, the more hasty decisions are justified.
...
In the condition wherein tokens jumped faster post-decision, unsurprisingly, the ratio of hasty decisions increased, moreover, the animals made faster movements too~\cite{Thura2014JNeurosci}.
% This suggests a common underlying mechanism that relates to motivation, reward expectancy and movement kinematics.
Therefore, the urgency signal may act as a determinant of the timing of decisions and the vigor with which those decisions are translated into actions in pursuit of maximal reward~\cite{Carland2019NeuroSci}.
...
% Why not all tasks show vigor deficit: automatic behavior is motivation-independent [Dudman2016CurrOpinNeurobiol]