...
In this section, all different conditions and tasks used in this works, as well as their technical details are described.
...
Rats had access to water everyday for 30~min after their experimental session, while their body weight was regularly measured.
...
A reward delivery port, connected to a solenoid valve, was installed on the front (relative to the turning direction of the belt) wall of the treadmill and released a $\sim$80~$\mu$L drop of 10\% sucrose water solution in case of a full reward.
...
In this condition, for each trial, the treadmill speed was pseudo-randomly drawn from a uniform distribution between 5 and 30~cm/s.
...
In this condition, the \gls{gt} was set to 3.5~s, half its value for the control condition.
...
Two different groups of animals were trained in this condition, one with treadmill speed set to the normal value of 10~cm/s, and another with the treadmill running twice as fast.
In the short goal time condition, we also examined if the increased variability in \gls{et} could be attenuated when the penalty associated with early \glspl{et} was increased and when reward magnitude was decreased for late \glspl{et}.
This was implemented by doubling the treadmill speed during the penalty period (from 10~cm/s to 20~cm/s), and delivering the reward for a narrower window of \glspl{et} (maximal reward at ET~$=$~GT~$=3.5$~s, and no reward after ET~$=4.5$~s).
...
\end{itemize}
The other task parameters were identical to the normal treadmill task.


\subsection{Locomotion Task} \label{ch:methods:loco}
A group of animals with a striatal lesion ($n=7$ DLS, $n=2$ DMS, and $n=3$ DS), and another group of non-lesioned animals ($n=12$) were used in this test to assess their general locomotor abilities (\Autoref{fig:lesion:motorOk}{A-B}).
