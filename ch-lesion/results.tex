...
Across training sessions, each composed of $\sim$130~trials, animals learned to wait longer and longer to enter the reward area just after the \gls{gt} and achieve higher percentage of correct trials (\Autoref{fig:lesion:task}{B}, and \autoref{fig:appendix:CorrectTrialCurve}).
...
This can be seen in example animals of \Autoref{fig:lesion:task}{F,~G}, and at the population level in \Autoref{fig:lesion:task}{H}, however the case illustrated in \Autoref{fig:lesion:task}{E} is an example of a rat with relatively smaller lesion and no apparent acute condition.
...
In addition, the difference in speed after the lesion was strongly correlated with the size of the lesion (\Autoref{fig:appendix:spd}{A}).
...
The number of times they licked after a reward delivery and also their peak lick frequency was not affected (\Autoref{fig:lesion:lick}{C-D}).
...
Therefore, another group of animals were trained in this version of the task and learned to proficiently perform the task by adopting the run-and-wait routine described above (\Autoref{fig:lesion:rev}{A}).
...
Rats with striatal lesions had an identical amount of displacement on the treadmill compared to the control ones (\Autoref{fig:lesion:motorOk}{A}).
...
Same groups of animals as in the last test ($n=12$ rats in each group) performed similarly in this free running task (\Autoref{fig:lesion:motorOk}{B}).
...
Our results indicate that the striatum is not required for the execution of motor routines (at least those similar to the wait-and-run), rather it is influencing kinematics of learned behaviors.
...
This effect that was well-correlated with lesion size as well (\Autoref{fig:appendix:spd}{B}).
...
Since the striatal lesion spared the rats' ability to learn the motor routine, execute it, and modulate their running speed, the most parsimonious account of our results is that lesioned animals \textit{preferred} slower speeds.
...
We found that higher effort sensitivity, regardless of the exact cost function parameters, resulted in optimal trajectories with smaller ``maximum position'', i.e., earlier run-phase initiation (\Autoref{fig:lesion:maxPos}{A}).
Hence, a combination of late initiation of the run bout together with fast running speeds was not used.
This result is not only in agreement with the reduced running speed observed following the lesion, but is also reminiscent of the behavior observed during the very first post-lesion sessions, when animals with larger \gls{ds} lesions ran mainly close to the reward area (\Autoref{fig:lesion:task}{F,~G}, and also \autoref{fig:appendix:Frontal}).
...
Also, the reduction in maximum position was well-correlated with the lesion size, the bigger the lesion, the bigger the reduction (\Autoref{fig:lesion:maxPos}{E}).
