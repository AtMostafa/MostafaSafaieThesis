...
Accordingly, we found that, globally dS lesions performed before training did not compromise the rats' ability to learn the wait-and-run routine (fig. S7).  
A few animals with a large dS lesion failed to display performance improvement, but strikingly, all of these animals ran all the time in the front region of the treadmill, as expected if they were reluctant to perform effort full fast locomotion bouts.
In animals with dS lesion performed before training, a robust reduction of running speed was also observed, an effect that was also correlated with lesion size (fig. S4). 
