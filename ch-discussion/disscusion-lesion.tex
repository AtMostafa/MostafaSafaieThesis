...
\begin{enumerate}[noitemsep, label=\Roman*.]
    ...
    This, once again, is in line with our hypothesis, i.e., lever pressing in the absence of the `skeptic' \gls{d2} neurons is more likely.\footnote{
        ``As activation in the direct pathway overpowers that of the indirect pathway, this imbalance accelerates the network toward `facilitation,' resulting in an executed action when the difference reaches a critical threshold''~\cite{Dunovan2016FrontNeurosci}.
    }
...
\end{enumerate}
\footnotetext{
    How manipulating the activity of \gls{da} neurons reflects in actual \gls{da} release in the striatum is another matter that seems to be non-trivial~\cite[for instance, see][]{Mohebi2019N}.
}
