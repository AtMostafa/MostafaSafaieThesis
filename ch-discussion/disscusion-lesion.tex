...
\footnotetext{
    For instance, the proposed function of the dorsal \gls{bg} to set the sensitivity to effort has implications for time estimation as well.
    ...
    Assuming a prokinetic role for \gls{da}, our framework (embodied timing, plus the effort function of the \gls{bg}), would suggest that mice executed a subtle motor routine during the to-be-estimated interval as a reference.
    The execution of this routine presumably was sped up upon \gls{da} photo-stimulation, and slowed down by \gls{da} inhibition (based on the \autoref{ch:disscusion:lesion:p1} above), causing overestimation and underestimation of the interval, respectively.
    This prediction, however in line with some previous reports (mentioned by the authors in~\cite{Paton2016Sci}), contradicts the results!
    One possible reason might be that how manipulating the activity of \gls{da} neurons is reflected in actual \gls{da} release in the striatum seems to be non-trivial~\cite[for instance, see][]{Mohebi2019N}.
}
