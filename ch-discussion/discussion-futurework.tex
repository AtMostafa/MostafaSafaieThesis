...
Indeed, we showed that better performance was correlated with more movement along the treadmill and visually, I observed that our best-performing animal developed a stereotyped ritual in interaction with the back wall of the treadmill.
...
Ideally, to test the embodied view, the time-estimation task should not allow any motor activity, perhaps by penalizing movements.
...
I speculate that direct (\textit{indirect}) pathway stimulation would cause cost underestimation (\textit{overestimation}), somewhat contrary (\textit{similar}) to what we observed by lesions.
